...
import com.idat.ecdanielmeza.service.UsuarioService;


@Controller
@RequestMapping("EC2-DanielMeza/usuario/v1") 
public class UsuarioController {
	...
	public @ResponseBody ResponseEntity<List<Usuario>> listar(){
		...
		return new ResponseEntity<List<Usuario>>(service.listarUsuarios(), HttpStatus.OK) ;		
	}
	...
	public ResponseEntity<Void> eliminar( @PathVariable Integer id  ) {
		...
		Usuario p = service.obtenerUsuarioId(id);
	...
	public ResponseEntity<Void> actualizar( @RequestBody Usuario usuario ) {
		...
		Usuario p = service.obtenerUsuarioId(usuario.getIdUsuario());
